...
export function freeEngineModels(data: unknown, providerID = 'opencode', accepts: (id: string) => boolean = id => freeModelIDs.includes(id)): ModelInfo[] {
  ...
  const models = object(all.find(item => object(item).id === providerID)).models;
  return Object.values(object(models)).flatMap(value => {
    ...
    if (typeof model.id !== 'string' || !accepts(model.id) || model.status === 'deprecated' || object(model.capabilities).toolcall !== true) return [];
...
export function engineCompletion(data: unknown, label = 'OpenCode'): Completion {
  ...
  if (info.error) {
    ...
    if (error.name === 'StructuredOutputError') throw new RouteError(`${label} did not return the required structured response. No Robinhood tools were executed. Try another model.`, 'protocol');
    if (status === 429) throw new RouteError(`${label} free model is rate-limited. Wait or select another model; remaining allowance is unknown.`, 'quota');
    if (status === 401 || status === 403) throw new RouteError(`${label} rejected anonymous access to this model. Select another advertised free model.`, 'auth');
    throw new RouteError(`${label} could not complete the request. Reconnect or choose another model.`, 'protocol');
  }
...
export interface ServerBridgeSpec {
  id: string; providerID: string; label: string; notice: string;
  models(data: unknown): ModelInfo[];
}
export class StructuredServerBridge implements Connector {
  private engine?: Engine;
  constructor(private readonly spec: ServerBridgeSpec, private readonly factory: () => Engine) {}
  ...
  async models(signal = AbortSignal.timeout(45_000)): Promise<ModelInfo[]> {
    try { return this.spec.models(await this.runtime().request('/provider', 'GET', undefined, AbortSignal.any([signal, AbortSignal.timeout(45_000)]))); }
...
export class OpenCodeBridge extends StructuredServerBridge {
  constructor(factory: () => Engine = () => new OpenCodeProcess()) {
    super({ id: 'opencode', providerID: 'opencode', label: 'OpenCode', models: freeEngineModels,
      notice: 'OpenCode CLI free-model bridge. Promotional models may retain data for training; use public, non-confidential data only. Allowance is unknown. OpenCode can retry internally within a two-minute deadline. No paid account credentials are loaded.',
    }, factory);
  }
}
