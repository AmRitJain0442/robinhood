...
import { engineCompletion, structuredSchema } from './opencode.js';
    ...
    return { id: `${this.spec.id}/${model}`, provider: this.spec.id, model,
      ...
      complete: async (messages, signal, onText, consent, options) => {
        ...
        try {
          ...
          const result = engineCompletion({ info: { structured } }, this.spec.label, tools);
