...
export async function delegateTask(store: Store, parent: Session, task: string, route: Route | Route[], ui: Interaction, secrets: Secrets, capabilities: Capabilities, signal: AbortSignal): Promise<Session> {
  ...
  try {
    await new Runner(store, secrets, capabilities).turn(child, `Independent research task: ${task}. Inspect and report findings; do not modify the workspace.`, (Array.isArray(route) ? route : [route]), ui, signal);
